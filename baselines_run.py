...
if __name__ == '__main__':
    ...
    args.dataset = 'dblp'
    modelname = 'HGNN1'#HGNN1,HNHN,GraphSAGE,GCN3P,HyperGCN
    best = []
    ...
    for i in range(10):
        ...
        L, nums, edge_dic, train_dic, test_dic, neg_test_dic, n_train, n_test ,user_item_matrix,templ= loaddata_link.readdata(args)
        ...
        model = HGNN1(nums,args.embed_size,128,64,num_heads=4,drop_rate=args.node_dropout[0]).to(args.device)#修改模型名字即可
    #  model = HyperGCN(nums, args.embed_size, 128, 64, drop_rate=args.node_dropout[0]).to(args.device)  # 修改模型名字即可
